...
local fluidMessage = {
	[FLUID_NONE] = "Urgh!", -- water 0
	[FLUID_SALTWATER] = "Urgh!", -- salt water 1
	...
	[FLUID_FRESHWATER] = "Gulp.", -- fresh water 21
	[FLUID_WHITEWINE] = "Aah...", -- white wine 22
	[FLUID_SILVER] = "Urgh!", -- silver fluid 23
	[FLUID_ROSEWINE] = "Aah...", -- rose wine 24
	[FLUID_DARKBLOOD] = "Urgh!", -- dark blood 25
	[FLUID_BERRYJUICE] = "Mmmmmh.", -- berry juice 26
	[FLUID_CHOCOLATEMILK] = "Mmmmmh.", -- chocolate milk 27
	[FLUID_ORANGEJUICE] = "Mmmmmh.", -- orange juice 28
}
...
function fluid.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if target.itemid == 26076 then
		...
		elseif item.type == 21 then
			toPosition:sendMagicEffect(CONST_ME_WATERSPLASH)
			target:transform(target.itemid + 1)
			item:transform(item.itemid, 0)
		else
			player:sendTextMessage(MESSAGE_FAILURE, "You need fresh water.")
		end
		return true
	end
	...
	if target.itemid == 1 then
		...
		elseif target.uid == player.uid then
			if table.contains({ 2, 3, 13, 16, 22, 24 }, item.type) then
