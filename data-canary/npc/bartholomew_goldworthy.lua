local internalNpcName = "Bartholomew Goldworthy"
...
npcConfig.voices = {
	interval = 15000,
	chance = 50,
	{ text = "Don't forget to deposit your money here in the World Bank before you head out for adventure.", yell = false },
}
...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "bank account") then
		npcHandler:say({
			"Every freeman or freewoman has one. \z
	...
	elseif MsgContains(message, "balance") then
		...
		if player:getBankBalance() >= 100000000 then
			npcHandler:say("I think you must be one of the richest inhabitants in the world! \z
				Your account balance is " .. player:getBankBalance() .. " copper.", npc, creature)
			return true
		elseif player:getBankBalance() >= 10000000 then
			npcHandler:say("You have made ten millions and it still grows! Your account balance is \z
				" .. player:getBankBalance() .. " copper.", npc, creature)
			return true
		...
		elseif player:getBankBalance() >= 100000 then
			...
			npcHandler:say("Your account balance is " .. player:getBankBalance() .. " copper.", npc, creature)
			return true
		end
		--Deposit
	elseif MsgContains(message, "deposit") then
		...
		if MsgContains(message, "all") then
			count[playerId] = player:getMoney()
			npcHandler:say("Would you really like to deposit " .. count[playerId] .. " copper?", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 2 then
		if MsgContains(message, "yes") then
			--[[if player:getStorageValue(Storage.Quest.U9_1.TheRookieGuard.Mission08) == 1 then
				...
				npcHandler:say("Alright, we have added the amount of " .. count[playerId] .. " +50 copper to your {balance} - that is the money you deposited plus a bonus of 50 copper. \z
			...
			if player:depositMoney(count[playerId]) then
				npcHandler:say("Alright, we have added the amount of " .. count[playerId] .. " copper to your {balance}. \z
	...
	elseif MsgContains(message, "withdraw") then
		if string.match(message, "%d+") then
			count[playerId] = getMoneyCount(message)
			if isValidMoney(count[playerId]) then
				npcHandler:say("Are you sure you wish to withdraw " .. count[playerId] .. " copper from your bank account?", npc, creature)
		...
		if MsgContains(message, "yes") then
			if player:getFreeCapacity() >= getMoneyWeight(count[playerId]) then
				if not player:withdrawMoney(count[playerId]) then
					...
					npcHandler:say("Here you are, " .. count[playerId] .. " copper. \z
	...
	elseif npcHandler:getTopic(playerId) == 14 then
		if getMoneyCount(message) < 1 then
			...
			count[playerId] = getMoneyCount(message)
			npcHandler:say("So you would like me to change " .. count[playerId] * 100 .. " of your copper \z
				coins into " .. count[playerId] .. " silver coins?", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 17 then
		if getMoneyCount(message) < 1 then
			...
			count[playerId] = getMoneyCount(message)
			npcHandler:say("So you would like me to change " .. count[playerId] .. " of your silver \z
				coins into " .. count[playerId] * 100 .. " copper coins for you?", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 19 then
		if getMoneyCount(message) < 1 then
			...
			count[playerId] = getMoneyCount(message)
			npcHandler:say("So you would like me to change " .. count[playerId] * 100 .. " of your silver coins \z
				into " .. count[playerId] .. " gold coins for you?", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 21 then
		if getMoneyCount(message) < 1 then
			...
			count[playerId] = getMoneyCount(message)
			npcHandler:say("So you would like me to change " .. count[playerId] .. " of your gold coins \z
				into " .. count[playerId] * 100 .. " silver coins for you?", npc, creature)
