...
function Npc:parseBankMessages(message, npc, creature, npcHandler)
	local messagesTable = {
		["money"] = "We can {change} metals for you. You can also access your {bank account}",
		["change"] = "There are three different coin types in this lands: \z
		...
		["bank account"] = {
			"Every freeman and every freewoman has one. The big advantage is that you can access your money in every branch of the World Bank! ...",
...
function Npc:parseBank(message, npc, creature, npcHandler)
	...
	elseif MsgContains(message, "deposit") then
		if string.match(message, "%d+") then
			...
			if isValidMoney(count[playerId]) then
				...
				npcHandler:say("You do not have enough pieces.", npc, creature)
				...
				npcHandler:say(
					string.format(
						"Alright, we have added the amount of %d copper to your {balance}. \z
                               You can {withdraw} your pieces anytime you want to.",
						count[playerId]
					),
					npc,
					creature
	...
	elseif npcHandler:getTopic(playerId) == 7 then
		if MsgContains(message, "yes") then
			...
			local goldCoins = math.floor(totalValue / 10000)
			totalValue = totalValue % 10000
			local silverCoins = math.floor(totalValue / 100)
			totalValue = totalValue % 100
			local copperCoins = math.floor(totalValue / 1)
			local goldPiles = math.floor((goldCoins + 99) / 100)
			local silverPiles = math.floor((silverCoins + 99) / 100)
			local copperPiles = math.floor((copperCoins + 99) / 100)
			local totalPiles = goldPiles + silverPiles + copperPiles
